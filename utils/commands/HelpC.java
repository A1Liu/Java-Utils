...
final class HelpC implements Command {
	 ...
	 * @return
	 */
	private ComTreeNode getNode(ComTreeNode root, String[] in) {//This is separate from the other method to allow for recursion-based handling of the skip command
		...
		for (int x = 0; x < in.length; x++) {
			...
			if (current.containsChild(element)) {//go to the next child if possible
				...
				subCommands = current.getChildren();
				return current;
			} else if (current.getID() != null && commands.get(current.getID()) != null && ( commands.get(current.getID()) instanceof SkipC ) ) {//We've come across a skip command.
	...
	/**
	 * The method gets help information from a node on the command tree and its children
