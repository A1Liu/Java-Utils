...
public class MergeSorter extends Sorter {
	...
	protected <T extends Comparable<T>> Accessor<T> sort0(Accessor<T> accessor, int startIndex, int endIndex) {
		 ...
		 * 
		 * Let A be the left range and B be the right range. First, find the first element in A that is > B_1. Then swap the B_1 with that element. 
		 * Then for every other element, swap with the correct element in A or B (favoring A). Each time an element from a range is swapped, the next element is selected by
		 * increasing the index for that range by 1. If the A index equals the B index, then the A index is set to the current swap position
		 * 
		 * 
		 *
		 * 
